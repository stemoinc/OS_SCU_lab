...
void history(HistoryArray* a) {
    ...
    int index = 1;
    do {
        printf("%d  %s\n", index, a->history_cmd[iterator]);
        iterator = (iterator + 1) % HISTORYSIZE;
        index++;
    } while (iterator != a->end);
}
void getHistory(HistoryArray* a, char* buf) {
    if (a->is_empty == 0) {
        a->is_empty = 0;
        for (int i = 0; buf[i]; i++) {
            a->history_cmd[a->end][i] = buf[i];
        }
        a->end = (a->end + 1) % HISTORYSIZE;
        if (a->end == (a->begin + 1)) {
            a->begin = (a->begin + 1) % HISTORYSIZE;
        }
    }else{
        a->is_empty = 0;
        for (int i = 0; buf[i]; i++) {
...
int check(char buf[], HistoryArray* hp) {
    char* history_a = "history";
    int is = 1;
...
int main(void) {
    ...
    hist.end = 0;
    hist.is_empty = 1;
    memset(hist.history_cmd[0], 0, 500);
    HistoryArray* hp = &hist;
